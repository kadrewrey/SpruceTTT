...
async function migrate() {
  ...
  try {
    ...
    const currentGameColumns = await db.execute(sql`
    ...
    const hasTextColumns = currentGameColumns.rows.some((col: any) => 
      ['winner', 'player_x', 'player_o'].includes(col.column_name)
    );
    ...
    if (hasTextColumns) {
      ...
      const hasUuidColumns = currentGameColumns.rows.some((col: any) => 
        ['winner_id', 'player_x_id', 'player_o_id'].includes(col.column_name)
      );
      ...
      const sampleGames = await db.execute(sql`SELECT * FROM games LIMIT 5`);
      ...
      console.log(`Games that couldn't be migrated: ${(unmigrated.rows[0] as any).count}`);
      
      if ((unmigrated.rows[0] as any).count > 0) {
        console.log('Some games could not be migrated. Showing problematic records:');
        const problematic = await db.execute(sql`
